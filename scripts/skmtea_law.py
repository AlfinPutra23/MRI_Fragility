...
for z in zs:
    ...
    for i in LAB:
        ...
        for R in RS:
            ...
            num=np.sqrt(((recon[R][M]-clean[M])**2).mean()); den=clean[M].mean()+1e-8
            acc[i][f"dg{R}"].append(float(num/den))     # per-structure NRMSE (degradation proxy)
f.close()
...
json.dump({"case":a.case,"rows":rows,"mech_pearson":pear[0],"mech_spearman":spear.correlation,
           "centroid_vs_degrR8_spearman":cs.correlation},open(f"{RES}/skmtea_law.json","w"),indent=2)
...
fig.suptitle(f"SKM-TEA {a.case}: faster scan → less energy kept (left) → lower fidelity (right)  —  both fall as R rises (higher = better)",fontsize=12,fontweight="bold")
fig.tight_layout(); fig.savefig(f"{PLOTS}/skmtea_law_predictors.png",dpi=140,bbox_inches="tight"); plt.close(fig)
...
ax.set_title(f"SKM-TEA {a.case}: the energy mechanism holds on REAL multicoil qDESS\nmore k-space energy kept  →  higher fidelity  (up-right = better; one curve for every structure × acceleration)",fontsize=11.5,fontweight="bold")
ax.legend(title="acceleration",frameon=False); ax.grid(alpha=.3)
fig.tight_layout(); fig.savefig(f"{PLOTS}/skmtea_law_mechanism.png",dpi=140,bbox_inches="tight"); plt.close(fig)
print("wrote: skmtea_law_predictors.png , skmtea_law_mechanism.png , skmtea_law.json")
