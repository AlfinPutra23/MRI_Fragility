...
Runs echoes 0 AND 1, overlays them. -> skmtea_law_v5_mechanism.png , skmtea_law_v5.json"""
...
def run_echo(e):
    ...
    for z in zs:
        ...
        for i in LAB:
            ...
            Fo=np.fft.fftshift(np.fft.fft2(img*M))                          # structure's own k-space (centered)
            for R in RS:
                For=Fo.copy(); For[:,masks[R]]=0                            # keep ONLY the removed lines
                errfield=np.abs(np.fft.ifft2(np.fft.ifftshift(For)))       # image-domain error this creates
                AER[i][R].append(np.sqrt((errfield[M]**2).sum()))          # v5: IN-REGION own error (leakage-corrected)
...
json.dump(res,open(f"{RES}/skmtea_law_v5.json","w"),indent=2)
...
for e in echoes:
    ...
    ax[0].set_title("INPUT: in-region removed-energy |a~_o| per structure (leakage-corrected)",fontweight="bold")
    ...
    fig.suptitle(f"SKM-TEA {CASE} — {ELABEL[e]}: more energy removed (left) → more error (right)   [v5 leakage-corrected, r={res[f'echo{e}_logr']:.2f}]",fontsize=12,fontweight="bold")
    fig.tight_layout(); fig.savefig(f"{PLOTS}/skmtea_law_v5_predictors_echo{e+1}.png",dpi=140,bbox_inches="tight"); plt.close(fig)
    print(f"wrote skmtea_law_v5_predictors_echo{e+1}.png")
...
ax.set_xlabel("in-region removed-energy error  |a~_o|  (leakage-corrected, log)   →  worse")
ax.set_ylabel("|reconstruction error|   (absolute amplitude, log)   ↑ worse")
ax.set_title(f"SKM-TEA {CASE}: the energy mechanism is ECHO-INVARIANT on real qDESS\n(v5: leakage-corrected in-region predictor)",fontsize=12,fontweight="bold")
ax.legend(frameon=False,fontsize=9,loc="lower right",bbox_to_anchor=(1,.12)); ax.grid(alpha=.3,which="both")
fig.tight_layout(); fig.savefig(f"{PLOTS}/skmtea_law_v5_mechanism.png",dpi=140,bbox_inches="tight"); plt.close(fig)
print("wrote skmtea_law_v5_mechanism.png , skmtea_law_v5.json")
