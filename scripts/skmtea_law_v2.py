...
for z in zs:
    ...
    clean=np.abs(img)
    lz=seg[:,:,z]
    gref=clean[lz>0].mean()+1e-8                          # v2: ONE global tissue reference per slice (brightness-neutral)
    for i in LAB:
        M=lz==i
        ...
        for R in RS:
            ...
            num=np.sqrt(((recon[R][M]-clean[M])**2).mean())
            acc[i][f"dg{R}"].append(float(num/gref))     # v2: GLOBAL-normalized error (not per-structure mean)
f.close()
...
json.dump({"case":a.case,"rows":rows,"mech_pearson":pear[0],"mech_spearman":spear.correlation,
           "centroid_vs_degrR8_spearman":cs.correlation},open(f"{RES}/skmtea_law_v2{SUF}.json","w"),indent=2)
...
fig.suptitle(f"SKM-TEA {a.case}: faster scan → less energy kept (left) → lower fidelity (right)  —  both fall as R rises  (v2: global-normalized fidelity, higher = better)",fontsize=12,fontweight="bold")
fig.tight_layout(); fig.savefig(f"{PLOTS}/skmtea_law_v2{SUF}_predictors.png",dpi=140,bbox_inches="tight"); plt.close(fig)
...
ax.set_title(f"SKM-TEA {a.case}: the energy mechanism holds on REAL multicoil qDESS\nmore k-space energy kept → higher fidelity  (v2: global-normalized, brightness-neutral)",fontsize=11.5,fontweight="bold")
ax.legend(title="acceleration",frameon=False); ax.grid(alpha=.3)
fig.tight_layout(); fig.savefig(f"{PLOTS}/skmtea_law_v2{SUF}_mechanism.png",dpi=140,bbox_inches="tight"); plt.close(fig)
print("wrote: skmtea_law_v2_predictors.png , skmtea_law_v2_mechanism.png , skmtea_law_v2.json")
