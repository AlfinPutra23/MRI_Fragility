...
\begin{enumerate}
  ...
  \item \textbf{A real-k-space mechanism}: the spectral energy removed by undersampling predicts the downstream task drop (Spearman $0.952$ over 24 structure-by-rate points; $0.943$ within fixed $R{\in}\{6,8\}$, $n{=}6$ structures, exact permutation $p{=}0.008$) --- a non-tautological energy$\rightarrow$task link beyond the Parseval identity.
...
We model a 2D multi-slice Cartesian acquisition. Each axial slice is transformed to k-space, undersampled along a single phase-encode direction by a 1D variable-density mask, and reconstructed by zero-filled inverse FFT. The mask retains a fully sampled autocalibration center of $8\%$ of phase-encode lines and draws the remainder without replacement under a polynomial density $(1-|x|)^{8}$ in the normalized distance $x$ from the center, to a total of $\lceil n/R \rceil$ lines at $R\in\{1,2,4,6,8\}$ ($R{=}1$ is the fully sampled baseline). \emph{One} mask is drawn per volume and applied to every slice, as in a single acquisition rather than a per-slice redraw. This yields SSIM/PSNR of $0.970/38$\,dB at $R{=}2$ falling to $0.745/27$\,dB at $R{=}8$. Masks are drawn with a generator seeded by a hash of the case identifier, so the degradation varies realistically across cases yet is deterministic and reproducible. Undersampling is simulated from magnitude images, as no public abdominal raw-k-space dataset with multi-organ labels exists; a control with a full complex multicoil forward operator ($A = M F S$) preserves the per-organ ordering (Spearman 0.978). Ground truth is always the annotation of the fully sampled series --- only the network's input is degraded.

\paragraph{Model and statistics.}
The segmenter is nnU-Net~v2, 3D full-resolution~\cite{isensee2021}, trained once at $R{=}2$ and deployed at every $R$ --- the practical setting of a fixed model meeting varying acceleration --- with no test-time augmentation. Per organ we report case-mean Dice, NSD, and HD95 over the 240 test series (Table~\ref{tab:surface}); the $R{=}1{\to}8$ Dice drop is tested with a paired Wilcoxon signed-rank test over cases, Holm-corrected across the 13 organs, with a paired Cohen's~$d$ as effect size. Because the 240 test series are four contrast phases of 60 patients, series are not independent: all bootstrap intervals resample \emph{patients}, and case-level tests are reported with that dependence stated. Series-level $p$-values are correspondingly anticonservative, so we re-ran the primary test on patient means: all 13 organs remain significant after Holm correction at $n{=}60$ patients (largest $p_{\text{Holm}}{=}2.9\times10^{-10}$; paired $d_z$ from $0.86$ to $2.71$).


\begin{table}[t]\centering\small
\caption{Surface metrics confirm and sharpen the Dice picture (MRISegmentator, $R{=}1$ vs.\ $R{=}8$,
case means over the 240 test series, organs sorted by Dice drop). Normalized surface Dice degrades
far more than volumetric Dice on the fragile organs --- the colon loses $0.041$ Dice-equivalents of
interior overlap but $0.355$ NSD and $28$\,mm of HD95 --- confirming that acceleration damage is
concentrated at organ boundaries, exactly where high radial frequencies live.}
\label{tab:surface}
\begin{tabular}{@{}lcccccc@{}}
\toprule
& \multicolumn{2}{c}{Dice} & \multicolumn{2}{c}{NSD} & \multicolumn{2}{c}{HD95 (mm)}\\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
organ & $R{=}1$ & $R{=}8$ & $R{=}1$ & $R{=}8$ & $R{=}1$ & $R{=}8$\\
\midrule
adrenal (L) & 0.638 & 0.440 & 0.655 & 0.444 & 7.5 & 17.2\\
adrenal (R) & 0.645 & 0.459 & 0.697 & 0.500 & 6.2 & 13.4\\
gallbladder & 0.895 & 0.725 & 0.835 & 0.522 & 5.1 & 14.2\\
colon & 0.932 & 0.769 & 0.865 & 0.510 & 12.5 & 40.5\\
duodenum & 0.858 & 0.717 & 0.782 & 0.553 & 10.6 & 16.7\\
esophagus & 0.869 & 0.741 & 0.903 & 0.747 & 4.1 & 7.0\\
small bowel & 0.940 & 0.826 & 0.885 & 0.632 & 6.9 & 23.0\\
pancreas & 0.908 & 0.835 & 0.829 & 0.664 & 4.5 & 11.4\\
stomach & 0.947 & 0.881 & 0.854 & 0.662 & 6.5 & 15.6\\
spleen & 0.969 & 0.921 & 0.853 & 0.715 & 8.3 & 8.4\\
kidney (L) & 0.969 & 0.929 & 0.884 & 0.752 & 4.0 & 7.0\\
kidney (R) & 0.974 & 0.935 & 0.912 & 0.776 & 3.4 & 15.3\\
liver & 0.989 & 0.970 & 0.938 & 0.823 & 4.6 & 9.7\\
\bottomrule
\end{tabular}
\end{table}
...
This single number rank-predicts an organ's Dice drop at $R{=}8$ with Spearman $\rho=0.863$ on MRISegmentator \cite{mrisegmentator2024} and $0.855$ on AMOS-MRI \cite{amos2022} (Fig.~\ref{fig:law}). The relation is stable under resampling: five-fold cross-validation gives $\rho = 0.841 \pm 0.023$, and a selection-corrected permutation test against four rival descriptors gives $p=0.0015$ (MRISeg) and $0.0053$ (AMOS). In pure rank terms the strongest geometric rival, surface-area-to-volume ratio, nearly ties in fold-CV ($0.830 \pm 0.023$), but calibrated prediction separates them: leave-one-organ-out cross-validated $R^2$ is $0.70/0.60$ (MRISeg/AMOS) for the centroid against $0.45/0.46$ for SA:V. The 240 test series are not independent: they are four contrast phases of 60 patients. Resampling \emph{patients} (all four phases of a patient moving together) puts the law at $\rho=0.863$ with a 95\% CI of $[0.797, 0.879]$; the naive series-level bootstrap gives the visibly narrower $[0.824, 0.879]$, and we report the patient-level interval throughout. Even so, we are explicit about what this interval covers: it addresses case-sampling noise \emph{only}. The inferential unit for the law itself remains the organ, and there are only 13 organs on MRISegmentator and 11 shared identities on AMOS; no bootstrap over cases enlarges that $n$. We state this limitation plainly rather than dressing the law in a CI it has not earned at the organ level.
...
The fragility law is a rank correlation over $n=13$ organs (MRISegmentator~\cite{mrisegmentator2024}) and $n=11$ shared organ identities (AMOS~\cite{amos2022}) --- a small sample of non-independent units drawn from a single body region. The patient-clustered bootstrap (resampling the 60 patients, since the 240 series are four phases each) gives a 95\% CI of $[0.797, 0.879]$ around $\rho=0.863$, but this addresses case-sampling noise \emph{only}; it does not enlarge the organ-level $n$, and no resampling scheme can. The 41 whole-body TotalSegmentator structures~\cite{wasserthal2023totalseg} and the cross-dataset replication mitigate, but do not remove, this limitation.
...
\paragraph{Sequence, field strength, and double acceleration.}
All abdominal results are T1-weighted abdominal MRI at the acquisition parameters of the source
datasets; the law and the released budgets are claimed for that regime, not for T2, DWI, or arbitrary
field strengths. The source images were themselves reconstructed by the scanner, typically with
vendor parallel imaging and filtering already applied, so our $R$ is acceleration \emph{relative to
the delivered image} rather than relative to a fully sampled raw acquisition; the budgets should be
read as additional retrospective acceleration on top of whatever the clinical protocol already used.
Finally, the $\tau{=}10\%$ tolerance defining $R^{*}$ is an operational threshold, not a clinically
validated one, and the budgets are cohort-level orderings rather than per-patient guarantees.

\paragraph{Architecture scope.}
The fragility \emph{ordering} is not an artifact of one network: a residual-encoder nnU-Net (ResEnc) reproduces the per-organ ordering of the standard configuration at Spearman $0.984$ over the 13 organs, and the centroid law holds on its drops (Pearson $0.870$). Both are nnU-Net variants, so the per-organ \emph{magnitudes} --- and the finding that mitigation is unavailable by post-hoc modeling at fixed acquisition and scale --- remain established for this architecture family and our training scale rather than universally.

\section{Conclusion}
\label{sec:conclusion}

Under k-space acceleration, multi-organ abdominal segmentation does not degrade uniformly: per-organ Dice loss spans an order of magnitude (adrenal $-0.198$ vs.\ liver $-0.019$; 13/13 organs significant after Holm correction, replicated on a second dataset), and a single aggregate Dice number hides all of it. This failure is predictable \emph{before any accelerated scan or segmenter is run}: a high-frequency anatomy signature --- the spectral centroid as its physical representative --- rank-predicts the drop at $\rho \approx 0.86$ on both datasets. The mechanism is grounded in real k-space: the energy an undersampling mask removes rank-predicts the downstream Dice degradation at $0.952$ over 24 structure-by-rate points and $0.943$ within fixed acceleration ($n{=}6$, exact permutation $p{=}0.008$) --- a task link that escapes the Parseval tautology, since image error and task loss are known to decouple~\cite{tolpadi2023,morshuis2025}.
...
\section*{Ethics}
This work is a secondary analysis of publicly available, de-identified imaging datasets
(MRISegmentator-Abdomen~\cite{mrisegmentator2024}, AMOS~\cite{amos2022},
SKM-TEA~\cite{desai2022skmtea}, fastMRI~\cite{zbontar2018fastmri}), each released by its original
custodians under its own data-use agreement and obtained in accordance with those terms. No new human
data were acquired, no patient contact occurred, and no attempt was made to re-identify any subject;
institutional review was therefore not required. All accelerations are simulated retrospectively from
already-acquired data, so no patient was scanned under a protocol studied here. The per-organ
acceleration budgets we release are cohort-level research findings on retrospective data and are not
validated for clinical protocol selection.

\section*{Data and code availability}
All code, the released per-organ acceleration budgets, the predictor implementation, the
undersampling-mask generator, and the serialized per-experiment result files backing every number in
this paper are available at \url{https://github.com/AlfinPutra23/MRI_Fragility}. The imaging datasets
are not redistributed; each is obtainable from its original custodian under that custodian's terms.

\section*{Funding and conflicts of interest}
This research received no specific grant from any funding agency in the public, commercial, or
not-for-profit sectors. The author declares no competing financial or non-financial interests.
